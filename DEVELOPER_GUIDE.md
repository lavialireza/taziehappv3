...
## اصلاحات پایدارسازی V1

- برای Field/Tazieh/Role/Section یک `stableKey` مستقل از ID داخلی Room اضافه شده است.
- فایل JSON می‌تواند برای هر سطح `id` پایدار داشته باشد. مبدل Word→JSON این شناسه‌ها را خودکار تولید می‌کند.
- `complete: true` در سطح تعزیه یعنی فایل مرجع کامل آن تعزیه است؛ بخش/نقشی که در نسخه جدید حذف شده باشد نیز از دیتابیس حذف می‌شود.
- ترتیب Role و Section مستقیماً از ترتیب آرایه JSON اعمال می‌شود.
- Backup نسخه 2 از stableKey برای Bookmark/Footnote/MyRole/Dialogue استفاده می‌کند و روی نصب تازه قابل بازیابی است.
- Deep Link جدید نیز با stableKey کار می‌کند و لینک عددی قدیمی برای سازگاری حفظ شده است.
- جستجوی فارسی از نرمال‌سازی حروف عربی/فارسی و FTS4 + fallback LIKE استفاده می‌کند.
- PDF برای Sectionهای بسیار طولانی به چند صفحه شکسته می‌شود.
- درخواست Notification در اولین اجرای برنامه حذف شده و ارسال اعلان فقط در صورت داشتن permission انجام می‌شود.
- فونت متن پیش‌فرض به Serif خواناتر تغییر کرده و B Titr برای تیترها استفاده می‌شود. برای استفاده دقیق از B Nazanin باید فایل فونت مجاز آن در `res/font` قرار گیرد.
- Build CI از Gradle 8.6 pinned استفاده می‌کند و `gradlew/gradlew.bat` به‌عنوان bootstrap wrapper اضافه شده است.
